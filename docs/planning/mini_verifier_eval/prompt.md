...
- The legacy FastAPI/Vite workbench has since been removed and replaced by the
...
Mobile/Citizen Jury state:

- The old portal revision was replaced by the Expo mobile/web app.
...
- The Expo app currently uses seeded frozen-baseline data while the protected
  CSV/audit import API is built.
- Restatement leakage guard runs before a card enters citizen review.
...
- Replace seeded Expo data with a protected CSV/audit import API.
- Add an explicit export/schema for training candidates and citizen labels
  before changing the classifier itself.
